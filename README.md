...
Filter out phantom clicks, mouse jitter, and scroll wheel bounces.
...
**Verbose mode** (shows all clicks, scrolls, and mouse movements):
```bash
./target/release/dewobble --verbose
```

**Combined** (hold mode with verbose output):
```bash
./target/release/dewobble --hold --verbose
```
